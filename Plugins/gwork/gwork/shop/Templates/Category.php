<?php
	...
	$categoryName = str_replace('%category%', htmlspecialchars($category->getRow()->name), $language['Category']['PageName']);
?>
				<h2 class="page-title"><?php echo $categoryName; ?></h2>
				<div class="row">
					<?php
						$products = $productFactory->getAllByColumn('category_id', $category->getRow()->id, false, null, '=', 'id', OrderTypes::DESC);
					...
					<div class="col-lg-3 col-md-4 col-xs-12">
						<div class="product-item">
							<?php
								if(strlen($product->getRow()->info_tag) > 0) {
							?>
							<div class="product-tag"><?php echo htmlspecialchars($product->getRow()->info_tag); ?></div>
							<?php
								}
							?>
								...
								<div class="row p-left-right-15 margin-top">
									...
									<div class="col-xs-8 col-lg-8 only-p-left">
										<a href="<?php echo SITE_PATH; ?>/product/<?php echo $product->getRow()->id; ?>" class="btn btn-secondary btn-block">
											<?php echo $language['Category']['MoreInfos']; ?>
										</a>
									</div>
								</div>
							</form>
